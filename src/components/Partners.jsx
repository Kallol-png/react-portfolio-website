        ...
        <div className='flex animate-marquee whitespace-nowrap overflow-hidden'>
            {
                duplicatedPartners.map((partner,index)=>(
                    <img key={index} src={partner.image} alt={partner.name} className='h-8 mx-6 object-contain min-w-[120px] hover:scale-110 opacity-50 transition-transform duration-200 hover:opacity-100'/>
                ))
            }
            {
                duplicatedPartners.map((partner,index)=>(
                    <img key={`duplicate-${index}`} src={partner.image} alt={partner.name} className='h-8 mx-6 object-contain min-w-[120px] hover:scale-110 opacity-50 transition-transform duration-200 hover:opacity-100'/>
                ))
            }
        </div>
    </div>
